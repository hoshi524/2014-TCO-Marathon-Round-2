				...
				for (int i = 0; i < map.length; i++) {
					for (int j = 0; j < map[0].length; j++) {
						if (map[i][j] > 0) {
							...
							g.fillRect(getX(XS[i]), getY(YS[j + 1]), getX(XS[i + 1]) - getX(XS[i]) + 1, getY(YS[j])
									- getY(YS[j + 1]) + 1);
						}
					}
				}
		...
		for (int i = 0; i < N; i++) {
			for (int j = i + 1; j < N; j++) {
				if (overlap(LX[i], RX[i], LX[j], RX[j]) && overlap(LY[i], RY[i], LY[j], RY[j])) {
					System.err.println("ERROR: rectangles " + i + " (0-based) and " + j
							+ " (0-based) in your solution overlap. Rectangle " + i + " is (" + LX[i] + ", " + LY[i]
							+ ") - (" + RX[i] + ", " + RY[i] + "). Rectangle " + j + " is (" + LX[j] + ", " + LY[j]
							+ ") - (" + RX[j] + ", " + RY[j] + ").");
					return -1;
				}
			}
